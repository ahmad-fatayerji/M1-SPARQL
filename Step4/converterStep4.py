...
def add_place_triples(g, subject_uri, city, country, predicate, place_ns, dbo, dbr, schema):
    ...
    place_name = f"{city}_{country}".strip("_")
    place_uri = URIRef(place_ns + safe_uri_component(place_name))
    ...
    if country:
        g.add((place_uri, RDFS.label, Literal(normalize_country(country), lang="en")))
